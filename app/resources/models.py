...
class ResourceQuerySet(models.QuerySet):
    ...
    def _apply_postgresql_search(self, query: str):
        vector = (
            ...
            + SearchVector("snapshots__page_title", weight="B")
            + SearchVector("snapshots__site_name", weight="C")
            + SearchVector("snapshots__ai_summary", weight="C")
            + SearchVector("snapshots__ai_category", weight="C")
    ...
    def _apply_fallback_search(self, query: str):
        return (
            self.filter(
                ...
                | Q(snapshots__page_title__icontains=query)
                | Q(snapshots__site_name__icontains=query)
                | Q(snapshots__ai_summary__icontains=query)
                | Q(snapshots__ai_category__icontains=query)
